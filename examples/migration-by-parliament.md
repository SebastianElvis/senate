...
A `parliament` debate is the right format when the question is *open* — *"should we do X?"* — and you want diversity of perspective with a recorded vote and dissent. Multiple parties argue from different angles, then vote. The speaker tallies and writes the verdict.
...
- **consensus** — *"design the migration plan"* — converging on a document, not a decision.
...
- **Re-running until you like the answer.** Don't. If you disagree with the verdict, run an `appeals-court` on it, or run a `consensus` to design the migration plan and let the disagreements surface there.
...
- **Plan the next debate.** A migration decision usually triggers a follow-up *"how do we do it?"* — that's a `consensus` (design the migration plan) or a `design-review` pipeline (oracle → committee → peer-review ‖ red-team → committee). Parliament is the *whether*; the *how* is downstream.
