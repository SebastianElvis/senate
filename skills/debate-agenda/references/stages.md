...
- **Explicit naming of a multi-stage pipeline:** `draft-review-finalize`, `design-review`, `bill-to-law`, `incident-post-mortem` — use the canonical recipes below. Pipeline recipes are stage sequences, not separate files in `../formats/`.
...
Pipeline recipes expand into `agenda.md` with `mode: pipeline`. They do not have standalone format files; every stage's `format` points to one of the format files in `../formats/`.

### draft-review-finalize

Use for documents that need draft → independent review → revision → acceptance.
...
stages:
  ...
  - index: 2
    ...
    roster:
      ...
      - { role: reviewer, cli: kimi }
      - { role: editor, cli: claude }
    input_bindings: [draft_doc]
    output_bindings:
      - { name: reviewed_doc, source: "verdict.md body" }
      - { name: review_decision, source: "fenced-json.decision" }
  ...
  - index: 3
    ...
    roster:
      - { role: member, cli: claude }
      - { role: editor, cli: claude }
    input_bindings: [reviewed_doc, draft_doc]
    output_bindings:
...
Verdict shape inside `## Final deliverable`: Final Document, Process summary, Review decision.

Failure notes: if the review decision is `reject`, pause at the conditional checkpoint; if `major_revision`, continue but warn the synthesize stage.

### design-review

Use for technical designs where the draft needs both independent review and adversarial pressure.

Defaults: `lead: claude`, `reviewers: [codex, gemini, kimi]`; budget `wall_clock_sec: 5400`, `total_tokens: 800000`.
...
Use for policy-shaped decisions where legitimacy of process matters: draft, distributed review, parliamentary vote, final form.
...
stages:
  ...
  - index: 2
    ...
    roster:
      ...
      - { role: reviewer, cli: "{public[0]}" }
      - { role: reviewer, cli: "{public[1]}" }
      - { role: reviewer, cli: "{public[2]}" }
      - { role: editor, cli: "{sponsor}" }
    input_bindings: [bill_text]
    output_bindings:
      - { name: public_feedback, source: "verdict.md body" }
      - { name: revised_bill, source: "transcript.author.last" }
      - { name: review_decision, source: "fenced-json.decision" }
    checkpoint: conditional
    condition: "stage.bindings.review_decision == \"reject\""
  ...
  - index: 3
    ...
    input_bindings: [revised_bill, public_feedback]
    output_bindings:
...
Failure notes: if vote fails, the checkpoint decides whether to archive or redraft; if peer review rejects the bill, parliament does not run.

### incident-post-mortem

Use for blameless incident review: identify systemic causes, draft remediations.

Defaults: `lead: claude`, `attackers: [codex, kimi]`, `defender: claude`, `judge: gemini`; budget `wall_clock_sec: 3600`, `total_tokens: 500000`.
...
stages:
  - index: 1
    ...
    output_bindings:
      ...
      - { name: outstanding_risks, source: "verdict.md section Outstanding risks" }
  ...
  - index: 2
    ...
    roster:
      - { role: member, cli: "{lead}" }
      - { role: editor, cli: "{lead}" }
    input_bindings: [root_causes, contributing_factors, outstanding_risks]
    output_bindings:
      - { name: remediation_plan, source: "verdict.md body" }
    checkpoint: none
```

Verdict shape inside `## Final deliverable`: Root Causes, Contributing Factors, Outstanding Risks, Remediation Plan.
...
```
1. committee (draft)
2. peer-review (independent review)
...
```
1. red-team (find root causes)
2. committee (write remediations)
```

### Design → critique → revise

```
1. committee (converge on a design)
2. red-team (attack it)
3. committee (revise informed by the attacks)
```
...
- **Insert** a stage (e.g., add a `committee` stage to synthesize a stalled `peer-review`).
