...
Triggered only if the moderator detects divergent answers (experts fundamentally disagree on a factual point).
...
If the moderator cannot auto-detect divergence (hard to do reliably), skip this phase.
...
- **Isolation is non-negotiable.** If the moderator cannot guarantee that experts do not see each other's turns, do not claim this format produces independent answers.

This file uses "moderator" to mean `moderate-debate`, "orchestrator" only to mean `senate`, and "synthesizer" to mean the `synthesizer` role within the format (not `meeting-note`, which is the post-run scribe).
