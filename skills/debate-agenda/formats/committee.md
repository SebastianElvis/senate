...
Produces the synthesis (which, for a committee, *is* the deliverable). The moderator writes it to `stages/<N>/verdict.md` (multi-stage) or the synthesis turn's text feeds `meeting-note` (single-stage):
