...
Every phase that needs to be machine-parsed declares a contract. The orchestrator validates each agent's reply against the contract; on failure, re-prompt once with the contract restated; on second failure, apply the format's fallback rule (usually: abstain / forfeit / reuse last draft).
...
Contracts live inline in the format file that requires them. When the orchestrator reads a format file, it also reads the contract(s) referenced there.
...
Every contract must ship with a re-prompt template. The template is itself a string; orchestrator fills in the agent's previous reply:
