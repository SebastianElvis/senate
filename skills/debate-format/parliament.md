...
| `speaker` | Orchestration role. Does not argue. Tallies votes, resolves ties, writes `verdict.md`. |

A minimal parliament has 2 MPs (one pro, one con) + speaker. Typical is 3 MPs + speaker. Additional parties ("mp_pragmatic", "mp_radical", etc.) can be added in the roster — give each a named brief in the role column of `roster.json`.
...
Output contract: markdown with the three sections above. Written to `.senate/runs/<id>/verdict.md`.
...
- **Agent failure**: a missing vote counts as `abstain`; missing rebuttals are skipped and noted in `verdict.md`.
