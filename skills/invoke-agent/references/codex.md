...
- No first-class token cap flag. Enforce budget in the prompt ("Respond in at most N words.") and with `timeout`.
...
- Exit code 0 with empty stdout has been seen on some model overload errors — treat empty stdout as a failure and retry once.
