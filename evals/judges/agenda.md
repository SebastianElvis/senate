...
1. **Format fit** — does the chosen format match the task shape? (Open policy → parliament; adversarial review → court; failure-mode hunt → red-team; review a submission → peer-review; produce a document → committee; ideation → brainstorm.) A reasonable mismatch (e.g., "court" for a brainstorm) scores 1–2.
