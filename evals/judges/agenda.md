...
1. **Format fit** — does the chosen format (and preset, where applicable) match the task shape? (Open policy → parliament; adversarial review → court; converge on artifact → workshop:consensus; etc.) A reasonable mismatch (e.g., "court" for a brainstorm) scores 1–2.
